...
def build_observations():
    """(cohorts, obs) - shared by the dynasty-value study and the redraft join."""
    ...
    print(f"{len(cohorts)} cohort pairs")
    ...
    for start, end in cohorts:
        ...
        for pid, p in a.items():
            ...
            obs.append({"year": start.year, "pos": p["pos"], "age": int(p["age"]),
                        "fp_id": pid, "start": start.isoformat(), "end": end.isoformat(),
...
cohorts, obs = build_observations()
