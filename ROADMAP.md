...
- **In-season logic - v1 scope AGREED 2026-08-20** (replaces the earlier "record takes
  ...
  2. **Playoff pace - BUILT 2026-08-21** (LOGIC.md "Playoff pace"): schedule-aware
     (real posted pairings), WEEKLY_MARGIN_SD measured on 2025 XFL (36). The per-week
     optimal-lineup reconstruction ALSO SHIPPED 2026-08-21 (Sleeper's weekly
     projections carry byes; `_weekly_strengths`). Still open: per-league margin SD
     from each league's own prior season; division rules (auto-bids per division, Sleeper's
