...
with its best sellable production, the UPGRADE SWAP (the starter the target would
...
it is pre-filtered by who would move him. And a BUY may not make the buyer meaningfully
worse: every buy-lens idea runs the framer's own lineup test for the buyer (~3ms) and is
dropped below -1.0 a game (`BUYER_DIP_OK`). Parker Washington + a 2nd for Hurts is the
calibrating case: -0.9 for kb (Hurts over Dart +1.1, Washington leaving kb's weak WR
...
production piece's dynasty price, so a second discount there is double-counting - the
earlier 0.7x floor for "production-priced" pieces produced Cam Ward alone for
Jonathan Taylor (0.69x; owner: "a slap in the face") and Price + a 2nd for Taylor
("probably wants to be a first"). Outside the band the LIGHT side sweetens from what
it would move, singles then pairs, cheapest that lands first: a buyer adds 1sts and
2nds only (3rds and 4ths aren't currency for a real chip - "at least a 2nd"; not a 1st
when a 2nd does it; same-round picks cost the same so the NEAREST year goes first, a
'27 1st is what people actually offer, not the '29 - owner) and only to a receiver that
